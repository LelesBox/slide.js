(function(global) {
    ...
    function newSlide(id, obj, interval, ready) {
        if (dom.isReady) {
            createDOM(id, obj, interval||3, slidecount);
            slidecount++;
        } else if (ready) {
            createDOM(id, obj, interval||3, slidecount);
            slidecount++;
    ...
    function createDOM(id, obj, interval, index) {
        ...
        each(imglist, function(item, index) {
            ...
            item.onclick = function() {
                var imgobj = obj[index];
                if (!startWith(imgobj.href, "http")) {
                    imgobj.href = "http://" + imgobj.href;
                }
                if (imgobj.isNewTag || false) {
                    window.open(imgobj.href);
                } else {
                    window.open(imgobj.href, "_self");
                }
            }
        })
